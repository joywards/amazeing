...
impl Renderer {
    ...
    pub fn initialize(&self, renderer: &mut Target) {
        if renderer.textures.light.is_none() {
            let light_surface = create_light_surface(&self.args).unwrap();
    ...
    pub fn render(&self, scene: &Scene, renderer: &mut Target) {
        ...
        if !self.args.debug {
            ...
            light_center.0 += self.args.cell_size as i32 / 2;
            light_center.1 += self.args.cell_size as i32 / 2;
    ...
    fn render_current_layer(&self, canvas: &mut Canvas, scene: &Scene) {
        ...
        let cells_iter: Vec<_> = if self.args.debug {
        ...
        for cell in cells_iter {
            if layer.has(cell) {
                ...
                let br = visual_info.map(|info| info.brightness)
                    .unwrap_or(self.args.invisible_cells_brightness);
                ...
                self.fill_rect(canvas, view_coord.0, view_coord.1, self.args.cell_size - 1, self.args.cell_size - 1);
                if layer.passable(cell, Dir::DOWN) {
                    self.fill_rect(canvas, view_coord.0, view_coord.1, self.args.cell_size - 1, self.args.cell_size);
                }
                if layer.passable(cell, Dir::RIGHT) {
                    self.fill_rect(canvas, view_coord.0, view_coord.1, self.args.cell_size, self.args.cell_size - 1);
                }
            }
        }
    }
    ...
    fn to_view(&self, scene_coord: (i32, i32), scene_camera: Camera) -> (i32, i32) {
        ...
        let x = (scene_coord.0 as f32 - scene_camera.0) * self.args.cell_size as f32 + view_camera.0;
        let y = (scene_coord.1 as f32 - scene_camera.1) * self.args.cell_size as f32 + view_camera.1;
        (x as i32, y as i32)
    }
...
fn create_light_surface(args: &Args) -> Result<Surface<'static>, String> {
    let radius = ((visibility_radius() as f32 - 1. / 2_f32.sqrt()) * args.cell_size as f32) as u32;
    let size = visibility_radius() as u32 * 2 * args.cell_size;
    let center = size / 2;
